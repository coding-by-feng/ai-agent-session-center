...
| `src/components/terminal/TerminalContainer.tsx` | Composes toolbar + xterm + bookmarks + fullscreen overlay; hosts `SelectionPopup`, gates translate-answer to Claude origins, and accepts `onClone?: () => void` |
...
- Fork (toolbar `onFork`): POST /api/sessions/:id/fork for Claude and Codex sessions. Claude commands use `--resume/--continue --fork-session` and preserve the source session's `permissionMode` via `reconstructPermissionFlags` (e.g. `--dangerously-skip-permissions`, `--permission-mode auto-edit|full-auto`). Codex commands use `codex fork <SESSION_ID>` or `codex fork --last`. On success, the forked session is auto-selected and assigned to the same room as the source session (via `roomStore.addSession`). **This is the in-place fork path — distinct from the floating-fork below.**
- Floating-fork (translate / explain): TerminalContainer's `handleTranslateAnswer` POSTs to `/api/sessions/spawn-floating` with `{ originSessionId, mode: 'translate-answer', nativeLanguage, learningLanguage }`, then opens the result via `useFloatingSessionsStore.open({ terminalId, label, originSessionId })`. The translate-previous-answer toolbar button is only passed when `detectCli(originSession) === 'claude'`, because that mode reads Claude transcripts. The select-to-translate popup uses the same store but with `mode='translate-selection'` / `'explain-selection'`. Floating-terminal hosts pass `originSessionId={null}` to suppress the popup and the translate-answer button (prevents recursion)
