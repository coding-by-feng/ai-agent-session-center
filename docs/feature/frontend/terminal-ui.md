...
- Fork: POST /api/sessions/:id/fork with --continue --fork-session, only for Claude sessions. On success, the forked session is auto-selected and assigned to the same room as the source session (via `roomStore.addSession`)
