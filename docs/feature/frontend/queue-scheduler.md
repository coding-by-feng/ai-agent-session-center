...
- **`ChainGate`** (per-session ref): `{ itemId, sawWork, openedAt, activityAtOpen? }` — holds the next chain step until the prior step's turn has visibly finished.
- **`OnceGate`** (per-session ref): `{ sawWork, openedAt, activityAtOpen? }` — same completion semantics as `ChainGate` but NOT keyed to an item id; sequences independent `once` items so a queue of several `once` prompts drains one-at-a-time (the next fires only after the previous one's task reaches Stop), instead of flooding the CLI in an ~1s burst.
- **Acceptance is dual-signal.** A gate treats the prompt as acknowledged when EITHER `sawWork` (a tick observed a busy status) OR `session.lastActivityAt > activityAtOpen` (the server stamped a hook event after the send — `server/sessionStore.ts:594` stamps every event). `sawWork` alone is sampled by a 1Hz `setInterval`, so a prompt whose whole busy phase lands between two ticks is missed; the activity stamp is event-driven and cannot be. A stale/equal stamp is not acceptance — it predates the send.
...
### Constants & values
- `NO_WORK_FALLBACK_MS = 300_000` (**exported from `queueScheduler.ts`**, imported by `useGlobalQueueScheduler.ts`) — last-resort gate release for a prompt the CLI never acknowledges at all (no busy status AND no hook event — e.g. `autoEnter` off, so the text sits unsent in the input line). **Its length is load-bearing.** It was `12_000`, which is shorter than a hook-less busy phase: `/compact` emits no hooks, so status stays stale at `'waiting'` (sendable) and `lastActivityAt` freezes — the gate read as an unacknowledged no-op and released every 12s, dumping all queued prompts into the CLI's own input queue. It lives beside the gate logic (not in the hook) so tests can assert the real value; a test with its own local fallback passed against the buggy build.
...
- **`pickNext(items, now, sessionWaiting, idleGuard, sessionExcludeWindows?, blockedByPrompting?)`** — selects the one item to fire, in priority order, after filtering out `disabled` items:
  - **Short-circuit** — `blockedByPrompting` → return `null` (checked before everything, so even in-flight chains pause one tick rather than typing over a just-submitted prompt). The pure helper accepts this param, but production never reaches it through `pickNext`: the scheduler hook enforces the guard with its own early return (see tick step 4) and calls `pickNext` without the 6th arg — only `queueScheduler.test.ts` exercises the short-circuit directly.
...
- **`chainGateDecision(gate, pickId, atRest, sessionSendable, now, noWorkFallbackMs, sessionActivityAt?)`** → `'fire' | 'hold'`. No gate / different item → fire. Accepted (see dual-signal above) → fire only when `atRest` (status `'waiting'` = the genuine Stop signal); decayed `idle` does **not** count. Not accepted → fire if sendable past `noWorkFallbackMs`, else hold.
- **`onceGateDecision(gate, atRest, sessionSendable, now, noWorkFallbackMs, sessionActivityAt?)`** → `'fire' | 'hold'`. Identical logic to `chainGateDecision` minus the item-id match — used to sequence consecutive `once` items: after one `once` fires, the next is held until the prior one's turn reaches `atRest` (Stop), with the same `noWorkFallbackMs` escape for a truly unacknowledged prompt.
...
9. On success: set 800ms cooldown, run `advanceAfterFire`, then `remove` / `updateItem(patch)` on the queue store. On `continue`, open a fresh chain gate `{itemId, sawWork:false, openedAt:now, activityAtOpen: session.lastActivityAt}`. On `remove` of a `once`, open a fresh once gate `{sawWork:false, openedAt:now, activityAtOpen: session.lastActivityAt}` so the next `once` holds until this one's task finishes. `activityAtOpen` is sampled from the tick's session snapshot, i.e. **pre-send** — any later stamp proves the CLI took the prompt. Emit a session-prefixed toast (e.g. `[name] Chain main sent (2 / 4)`, `Loop fired`, `Auto-sent queued prompt`).
