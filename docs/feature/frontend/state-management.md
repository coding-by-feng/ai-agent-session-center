...
## Function
Global state management via 14 Zustand stores providing reactive session, WebSocket, UI, settings, queue, queue-history, prompt-snippet, notes, camera, room, label, shortcut, agenda, and floating-sessions state.
...
| `src/stores/settingsStore.ts` | Extends `BrowserSettings`. 10 themes (THEMES), per-CLI sound profiles (CLI_SOUND_PROFILES: claude/codex), ambient (DEFAULT_AMBIENT_SETTINGS), label alarms, hookDensity, API keys (anthropic/openai/googleTts), TTS block (ttsEnabled, ttsVoiceEn, ttsVoiceZh, ttsSpeakingRate), translation/explain block (translationEnabled, translationNativeLanguage, translationLearningLanguage, translationTrigger `'auto'\|'alt'\|'off'`, translationInheritContext, explainAttachFilePath `'ask'\|'always'\|'never'`), terminal replay buffer (`terminalReplayBufferBytes`, default `DEFAULT_TERMINAL_REPLAY_BUFFER_BYTES`; `setTerminalReplayBufferBytes` clamps via `clampReplayBufferBytes` before persisting) and live xterm scrollback (`terminalScrollbackLines`, default `DEFAULT_TERMINAL_SCROLLBACK_LINES` = 5 000; `setTerminalScrollbackLines` clamps via `clampScrollbackLines`, range 1 000–200 000) — the first is server/main-side replay bytes, the second is renderer memory for an already-open terminal; see [Terminal UI](./terminal-ui.md). Complex store with DOM side effects (CSS vars, data-theme). Actions: per-CLI sound config, label alarms, setApiKey (provider includes `'googleTts'`), TTS/translation setters, loadFromDb/saveToDb, persistSetting, flashAutosave, resetDefaults |
| `src/stores/queueStore.ts` | `queues: Map<string, QueueItem[]>` + `automation: Map<string, QueueAutomationConfig>`. Actions: add/remove/reorder/moveToSession/setQueue/updateItem, automation setters (getAutomation/setPaused/setAutoSend/setAutoEnter/setIdleGuard/setSkipWhenPrompting/setAutoResume/setResumeMaxRetries/setResumePrompt/setLoopExcludeWindows/setAutomation (whole-config replace)), migrateSession, loadFromDb. Persist subscription writes queues→`db.promptQueue` and automation→`db.queueAutomation`. Scheduling/chain semantics live in [Prompt Queue](./prompt-queue.md) / [Queue Scheduler](./queue-scheduler.md) |
...
| `src/stores/promptSnippetStore.ts` | Curated saved-prompt library (🔖). `snippets: PromptSnippet[]`, own Dexie table `db.promptSnippets` (v7). **Nothing may write to it automatically** — only an explicit 🔖 promotes text into the library; raw prompt history stays a read-only pooled view. See [Saved Prompts](./saved-prompts.md) |
| `src/stores/notesStore.ts` | Cache of the server's per-session notes. `notes: Map<sessionId, SessionNote[]>` (newest first). Actions: loadNotes/addNote/deleteNote — each awaits the API and returns a boolean, mutating the Map only after the server accepted, so a failed call can't desync the NOTES count badge from SQLite. Rows are converted from the API's snake_case in one place (`fromRow`); requests use `authFetch`. Not persisted client-side — server SQLite is the source of truth. See [Session Detail Panel](./session-detail-panel.md) |
...
- `DEFAULT_AUTOMATION` is a frozen sentinel `{ paused:false, autoSend:true, autoEnter:true, idleGuard:true, skipWhenPrompting:true, autoResume:true, resumeMaxRetries:3, resumePrompt:'' }` returned for sessions with no config — selectors fall back to this stable reference to avoid a re-render loop.
