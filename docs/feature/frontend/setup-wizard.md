...
| `src/components/setup/steps/ConfigureStep.tsx` | Port, Codex toggle, hook density, history retention, optional password — `react-hook-form` + Zod |
...
| `server/hookInstaller.js` | `ensureHooksInstalled()` — runs on every server startup; density-aware hook registration for Claude/Codex |
