...
- **DbSession** schema: 21 fields (id, projectPath, projectName, title, status, model, source, startedAt, lastActivityAt, endedAt, totalToolCalls, totalPrompts, archived, summary, characterModel, accentColor, teamId, teamRole, terminalId, queueCount). Note: `totalPrompts` is derived from `promptHistory.length` at persist time.
...
- `deleteSession` / `migrateSessionId` cascade must stay in sync with `CHILD_TABLES`; remember `translationLogs`, `queueAutomation`, and `queueHistory` are deliberately excluded.
