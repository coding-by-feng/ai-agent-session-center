...
- **Submit** (`handleSubmit`): always `POST /api/terminals` with `forceNew: true` (NewSessionModal has no Electron-IPC branch). Claude sends its model + effort; Codex sends only `codexModel`; shell commands send neither, preventing a hidden Claude preference from leaking to another CLI. An empty Codex selection omits `model`, letting Codex track its recommended default. On success: saves workdir/command history, `lastSession` and per-dir config (`dir-session-configs`) — both now `{ workingDir, command }` only — session prefs, and remote-control settings; auto-selects the new session (`selectSession`) and assigns it to the chosen room.
