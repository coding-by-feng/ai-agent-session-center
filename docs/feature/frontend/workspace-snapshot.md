...
| `src/components/modals/RestorePickerModal.tsx` | Restore picker shown on every restart (skippable). Groups sessions by room, supports search + All/None/★Pinned, flags `⚠ new` (non-UUID — fresh conversation) and `⚠ fork` (`--fork-session` in command) per row, persists last selection and "Don't ask again" via `localStorage`. Module-level `requestRestoreSelection(snapshot)` returns `Promise<PickerResult>`. |
