...
| `src/hooks/useTerminal.ts` | Terminal-side trigger: the file-path `registerLinkProvider` (registered *after* the URL provider, single-line scope — unlike the URL provider it does not group wrapped lines) hands each match the hover tooltip `Click to choose how to open`, and its `activate(event)` opens the chooser at the mouse position (`event.clientX/clientY`) with the path cleaned of a leading `./` and `projectPathRef.current` (or `''`) as the project. Detection uses `createFilePathRegex()`; link column ranges come from `mapLineColumns()` so wide (CJK) characters stay aligned. |
