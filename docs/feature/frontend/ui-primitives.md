...
- **Grouped registry** (keys) covering icon buttons across the app:
  ...
  - TerminalToolbar: `termPopOut`, `termTheme`, `termSendEsc`, `termPaste`, `termSendUp`, `termSendDown`, `termSendEnter`, `termScrollBottom`, `termRefresh`, `termNewSession`, `termFork`, `termSpeak`, `termSpeakLocal` (offline on-device Kokoro voice — click-to-toggle, distinct from `termSpeak`'s hold-to-speak cloud TTS), `termReconnect`, `termAutoScrollOn`, `termAutoScrollOff`, `termBookmark`, `termClone`, `termFullscreen`, `termFullscreenExit`, `termThemePicker`.
