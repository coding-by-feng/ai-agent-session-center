...
The **TERMINAL**, **COMMANDS**, and **PROJECT** tabs can each be popped out into their own lightweight native OS window (Electron only, draggable to another monitor). TERMINAL/COMMANDS use a `⧉` toolbar button; PROJECT (float) uses the drag-to-edge gesture (no dedicated button). An in-app DOM panel is confined to the app window, so cross-monitor movement requires a real window. All popouts open a **content-only** window (just that tab's content, not the whole dashboard) loaded from `/?popout=…` — `main.tsx` detects the `popout` query param before bootstrapping `<App>` and renders the popout view directly:
...
| PROJECT (float) | drag-to-edge gesture | `electronAPI.openProjectWindow({ path, label })` → `/?popout=project&path=…` | `PopoutProjectView` (wraps `ProjectBrowserView`) |
...
**Project popout** (`FloatingProjectPanel.tsx`): a single trigger via the async `handlePopOut()` (returns whether a window opened) — the **drag-to-edge gesture** (drag the expanded panel `POPOUT_EDGE_THRESHOLD` = 56px past its clamped position in any direction → border glows `.floatPanelPopoutArmed` + a centered "⧉ Release to pop out" hint `.floatPopoutHint`; overshoot measured from the clamped edge so all four edges arm symmetrically; expanded-panel-only, never when maximized; gated on the session having a `projectPath` via `canPopOut`). There is **no dedicated `⧉` header button** — the gesture is the sole pop-out affordance (the title bar keeps only minimize / maximize / close). The gesture closes the in-app float on a confirmed-open window (`onClose` → float mode off) so the project isn't live in two places. Under Electron it awaits `electronAPI.openProjectWindow` (native `BrowserWindow` on a secondary monitor, bounds persisted, de-duped by path); in the browser it falls back to `window.open(/project-browser?path=…, name)` with a stable per-path window name.
