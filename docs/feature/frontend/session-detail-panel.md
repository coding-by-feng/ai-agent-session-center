...
- **Stacked view**: a third PROJECT-tab toggle (the two-rows icon, between split and float) shows Terminal **on top** and Project **below** via `StackedSplitView` — a vertical sibling of DraggableSplitView with a horizontal `row-resize` divider (ratio clamp 0.2–0.8). Each pane keeps the full panel width, so it fits the ~480px panel better than side-by-side. State `split-stacked-terminal-project:{sessionId}`, ratio `split-stacked-ratio:{sessionId}`. **Split, Stacked, and Float are 3-way mutually exclusive** — enabling one disables the other two (`toggleSplit`/`toggleStacked`/`toggleFloat` each clear the others via the shared `turnOff` helper). `effectiveTab` becomes `'stacked'` when stacked is on and the active tab is terminal/project; the always-mounted terminal host and project portal are suppressed in stacked mode (StackedSplitView renders its own single copies), exactly like split.
