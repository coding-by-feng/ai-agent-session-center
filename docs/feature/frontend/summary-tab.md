...
- **Context size.** `buildContext` concatenates the whole `promptHistory`/`toolLog`/`responseLog` it is given; very long sessions can exceed the server subprocess `maxBuffer` (1 MB) or the 60s `timeout`, causing `Summarize failed`. Since the memory-optimization pass, `promptHistory` is no longer the unbounded part: `server/sessionTrim.ts` caps it at `MAX_PROMPT_HISTORY_ENTRIES = 50` entries, `MAX_PROMPT_CHARS = 16_000` per entry, and `MAX_PROMPT_HISTORY_CHARS = 200_000` total (oldest dropped first, over-long text replaced with `TRUNCATION_MARKER`), so a summarized transcript reflects the **trimmed** prompts, not the originals — the full text lives in the SQLite `prompts` table (`db.insertFullPrompt`) and the CLI JSONL transcript. `toolLog` (200 entries) and `responseLog` (50 entries) are still only entry-count capped with no per-entry character budget, so they are the remaining overflow risk.
