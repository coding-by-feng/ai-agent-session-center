...
- **`Session.model` holds a model id — never a launch command.** The connecting→idle transition fills in a model for a hookless CLI via `extractModelFromCommand(command)` (config.ts: `--model <id>` / `--model=<id>` / `-m <id>`, run through `sanitizeModelId`, `''` when the command pins none), and only when `model` is still empty. The previous shortcut — `if (!command.startsWith('claude')) s.model = command` — stamped the **whole command** into the field for every non-Claude CLI, so live Codex cards carried `model = "/opt/homebrew/…/bin/codex --dangerously-bypass-approvals-and-sandbox"`. That value (a) rendered as a three-line wall of path in the card's model slot, overflowing the 230px left rail (see [Session detail panel](../frontend/session-detail-panel.md)), (b) would be re-emitted as an unquoted `--model <whole command>` by `applyClaudeLaunchFlags` on fork/resume/clone, (c) is silently dropped by `buildSnapshot`'s `^[a-zA-Z0-9._-]+$` filter, losing a pinned model on [workspace restore](../frontend/workspace-snapshot.md), and (d) overwrote a model the caller had just set at creation. The `startsWith('claude')` test was also path-blind, so `/usr/local/bin/claude` was treated as "some other CLI". Existing corrupted rows are cleaned by the [db.ts startup migration](./database.md). Covered by `test/sessionModelIntegrity.test.ts`.
...
- `cliSource?: string` records the originating CLI when hooks provide `cli_source` (the Codex hook emits it) or when a terminal is created from a recognizable startup command (`inferCliSource`). Frontend CLI badges prefer this field before guessing from model/event data, and the floating-popup spawner's `resolveOriginCli` reads it first so a popup inherits the parent's CLI.
...
- Transient-failure marker: `interruption?: SessionInterruption | null` — `{ kind: 'api_error' | 'rate_limit' | 'network', line, detectedAt }`, set when the session's PTY printed a failure banner (`server/interruptionDetector.ts`, wired via `registerTerminalFaultCallback`). It is **additive** — `session.status` is deliberately left alone, because the turn really did end and re-labelling it would break the queue's gates. **Cleared at the top of the hook-event switch on `UserPromptSubmit` / `PreToolUse` / `PostToolUse` / `SessionStart` — never on `Stop`,** because a 529 produces `Stop` exactly like a clean finish; clearing there would erase the fault before anything could act on it. This field exists because nothing else in the state machine can distinguish "finished the task" from "died on a provider error": both land in `waiting`. Consumed by [Auto-Resume Watchdog](../frontend/auto-resume-watchdog.md).
- External-session marker: `isExternal?: boolean` — flags a session the dashboard did NOT launch (a real Claude/Codex CLI running in an external terminal, or started before hooks were installed), so it never bound to a dashboard PTY (`terminalId` stays `null`). Terminal actions (open / reconnect / kill-via-PTY) do not apply. See *External Sessions (discovered / hook-only)* below for the two producers. `sessionMatcher` clears the flag (`isExternal = false`) when such a session is later adopted onto a dashboard terminal.
...
- [Auto-Resume Watchdog](../frontend/auto-resume-watchdog.md) — owns `session.interruption`: this module sets it from the terminal-fault callback, clears it on forward-progress hook events, and broadcasts it
