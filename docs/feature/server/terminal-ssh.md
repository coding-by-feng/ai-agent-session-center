...
## Purpose
Enables the dashboard to create interactive terminal sessions that connect to AI CLI processes. This is how users launch and monitor Claude/Codex.
...
### Output Ring Buffer
- **Configurable size, lazily grown** per terminal: `createRing(replayBufferBytes)` from [`server/ptyRing.ts`](../../../server/ptyRing.ts), where `replayBufferBytes` is a module-level variable (default `DEFAULT_TERMINAL_REPLAY_BUFFER_BYTES = 1 * 1024 * 1024` = 1 MB, raised from the former hard-coded 128 KB). The default + clamp constants live in `src/types/terminal.ts` (`MIN_TERMINAL_REPLAY_BUFFER_BYTES = 256 * 1024`, `MAX_TERMINAL_REPLAY_BUFFER_BYTES = 32 * 1024 * 1024`, `clampReplayBufferBytes()`).
...
- **The ring starts at `INITIAL_RING_BYTES` (64 KB) and doubles toward `outputRing.cap` as output arrives.** It used to be allocated at full size on create, so every terminal cost its whole cap (2 MB at the time) the instant it spawned. `outputRing.buf.length` is the **current allocation**, never the capacity — read `outputRing.cap` for that. INVARIANT: a ring only wraps once `buf.length === cap`, which is what makes in-place growth safe (live bytes are always the linear prefix `[0, offset)`).
...
- Only applies to commands starting with `claude` (Codex ignored)
- Skips if command already contains `-n` or `--name` flag
- `appendSessionName()` helper in `server/config.ts` shell-quotes the title and **normalizes** any `-n` already present (replace, not skip).
  - **The title is untrusted input.** `buildAutoTitle` derives it from the first 60 chars of the user's first prompt (`makeShortTitle` does no shell sanitizing) and workspace-snapshot import carries titles in from a shared JSON file — so prompt text reaches a live shell here. `quoteSessionTitle` escapes the four characters the shell still treats as special inside double quotes (`\`, `$`, `` ` ``, `"`) **with backslash first** (escaping `"` first would let a `\` + `"` pair forge a quote break), and collapses control characters via `CONTROL_CHARS_RE = /[\x00-\x1F\x7F]+/g` — a raw newline in a title would otherwise end the command line and turn the remainder into a second shell command. Escaping only `"` (the previous behaviour) left `$(…)`, backticks and `$VAR` fully live: a session titled ``a`id`b`` ran `id` on every resume, clone, fork and pinned respawn.
  - **An existing `-n` is replaced, not preserved.** The old "don't double-add" guard treated *any* existing flag as valid, so a single command carrying an unquoted `-n KTS Deployment` propagated through every later resume/clone/fork unrepaired — and `claude` kept taking `KTS` as the name and `Deployment` as a stray positional argument (which Claude Code reads as an *initial prompt*). Normalizing means stored commands self-heal on their next pass. Titles are still double-quoted rather than single-quoted so `stripClaudeSessionName` / `extractSessionName` keep round-tripping; both accept `\` escapes in the quoted value (`(?:[^"\\]|\\.)*`), and `extractSessionName` unescapes so a strip → re-append cycle is lossless. Covered by `test/sessionNameQuoting.test.ts`.
...
- API-key routing is command-aware for direct and path-qualified binaries: Codex receives `OPENAI_API_KEY`, and all other commands receive `ANTHROPIC_API_KEY`. The same `apiKeyEnvForCommand()` choice is used for local env objects and remote/tmux `export` commands.
...
- registerTerminalFaultCallback(cb) — register callback for **transient-failure banners** (`API Error: 5xx` / rate limit / connection error) seen in PTY output. The private `notePtyOutput(terminalId, data)` helper runs the detector inside both agent-terminal `onData` seams (right after `ringWrite`), and `cleanup()` calls `clearFaultState(terminalId)` so a reused terminal id starts with a clean carry/dedupe state. Detection is regex-only — no I/O, no process probe — and any throw is swallowed to `log.debug` so it can never break terminal streaming. sessionStore registers one to set `session.interruption`. See [Auto-Resume Watchdog](../frontend/auto-resume-watchdog.md).
