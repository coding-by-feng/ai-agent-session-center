...
- Auto-name counter is per-project (e.g. "agent-manager #1", "thesis #2")
...
| `writePty(id, data)` | Write data to PTY stdin |
...
`app.before-quit` -> `disposePtyHost()` kills all PTY processes and cleans up all subscriptions.
